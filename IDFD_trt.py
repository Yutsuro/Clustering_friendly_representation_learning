...
model_trt = torch2trt(net, [size], fp16_mode=True, max_batch_size=bs)
...
torch.save(model_trt.state_dict(), f'IDFD_trt_{str(bs)}.pth')



model_trt = TRTModule()

model_trt.load_state_dict(torch.load(f'IDFD_trt_{str(bs)}.pth'))