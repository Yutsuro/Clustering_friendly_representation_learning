...
def inference(model_path,batch_size):
    ...
    for inputs, _, _ in test_loader:
        with torch.no_grad():
            ...
            #inputs = inputs.to(device, dtype=torch.int8, non_blocking=True)
...
if __name__=="__main__":
    ...
    model_trt = "IDFD_trt_512_int8.pth"
    batch_list = [64,128,256,512]
    
    inference(model_trt,batch_size=512)
    ...
    for batch_size in batch_list:
        ...
        clu = []
        tot = []
        model_path = f"IDFD_trt_{batch_size}.pth"
        #model_path = f"IDFD_trt_{batch_size}_int8.pth"
        for _ in range(3):
